...
public class WindowSketchQuantiles implements IQuantiles<Double>, Serializable {
     ...
     */
    public WindowSketchQuantiles(double epsilon,int windowSize) {
        ...
        this.setWindowSize(windowSize);//32768
    ...
    public Double getQuantile(double q)  {
        ...
        System.out.println("elementCountSize1 "+this.elementCount+" windowSizeSize2"+this.windowSize);
        if (this.elementCount < this.windowSize){
            return this.initialGK.getQuantile(q);
        }

        //LinkedList<ComparableWrapper<Double, Float>> sortedSummary = this.getOrderedSummary();
        System.out.println("elementCountSize2 "+this.elementCount+" windowSizeSize2"+this.windowSize);
        LinkedList<Double> summary = this.getFinalSummary();

        System.out.println("summarySize1 "+summary.size());
    ...
    private LinkedList<Double> getFinalSummary(){
        ...
        System.out.println("getsummary "+summary.size());
        // for each block
        for (int i = 0; i < summary.size(); i++) {
            // for each quantile in the current block
            Float weight = this.computeLevelForEpsilon(summary.get(i).getEpsilon());
            System.out.println("weight "+weight);
            System.out.println("getQuantiles "+summary.get(i).getQuantiles().size()+" Total elements:"+elementCount);

            for (int j = 0; j < summary.get(i).getQuantiles().size(); j++) {
                for (int k = 0; k <= weight; k++) {
                    finalSummary.addAll(summary.get(i).getQuantiles());
                    System.out.println("getFinalSummary "+finalSummary.size()+" Total elements:"+elementCount);
                }
            }
        }
