 ...
 */
public class AMSSketch implements ISimpleFrequency<Double>, Comparable<AMSSketch>, Serializable {
    private int depth;
    private int buckets;
    private int count = 0;
    private int[] counts;
    ...
    public boolean add(Double item, long incrementCount) {
        int offset = 0, hash, mult;
        count += incrementCount;

        long itemAsLong = Double.doubleToRawLongBits(item);

        for (int j=0; j<depth; j++) {
            hash = (int) HashUtils.hash31(test[0][j], test[1][j], itemAsLong);
            hash = hash % buckets;
            mult = (int) HashUtils.fourwise(test[2][j], test[3][j], test[4][j], test[5][j], itemAsLong);
            if ((mult&1)==1)
    ...
    public long estimateCount(Double item) {
        ...
        for (int i=1; i<=depth; i++) {
            hash = (int) HashUtils.hash31(test[0][i-1], test[1][i-1], itemAsLong);
            hash = hash % buckets;
            mult = (int) HashUtils.fourwise(test[2][i-1], test[3][i-1], test[4][i-1], test[5][i-1], itemAsLong);
            if ((mult&1)==1)
