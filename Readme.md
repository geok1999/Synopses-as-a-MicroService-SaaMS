...
An example of how to use for each type of Synopsis the query and not query Request Message exist in the:
`RequestExamples` folder.
...
Producer messages for Request Topic:
```
...
```
Producer messages for Data Topic:
```
...
```
The `-DconfigFilePath="DataPath"` is optional. If it is not set, the application will use the default values to configure the App.
